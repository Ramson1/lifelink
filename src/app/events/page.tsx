          ...
          <div className="mt-12">
            ...
            {certificates.length > 0 ? (
              <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {certificates.map((cert, i) => (
                  <ScrollReveal key={cert.id} delay={i * 80}>
                    <div className="flex aspect-[4/3] items-center justify-center rounded-3xl border border-black/10 bg-white/70 p-6 text-center overflow-hidden">
                      {cert.image_url ? (
                        <Image
                          src={cert.image_url}
                          alt={cert.title}
                        ...
                        <div>
                          ...
                          <div className="mt-3 text-sm font-semibold text-black">{cert.title}</div>
                          {cert.subtitle && <div className="mt-1 text-xs text-black/60">{cert.subtitle}</div>}
                        </div>
                      )}
                    </div>
                  </ScrollReveal>
              ...
              <div className="mt-6 rounded-3xl border border-black/10 bg-white/70 p-8 text-center">
                <p className="text-sm text-black/60">Certificates will be displayed here soon.</p>
              </div>
            )}
          </div>
        </ScrollReveal>
          ...
          <div className="mt-16">
            ...
            {events.length > 0 ? (
              <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {events.map((ev, i) => (
                  <ScrollReveal key={ev.id} delay={i * 80}>
                    <div className="group overflow-hidden rounded-3xl border border-black/10 bg-white/70 transition hover:shadow-lg">
                      {ev.image_url && (
                      ...
                      <div className="p-5">
                        ...
                        {ev.description && (
                          <p className="mt-1.5 text-sm leading-6 text-black/70 line-clamp-3">{ev.description}</p>
                        )}
              ...
              <div className="mt-6 rounded-3xl border border-black/10 bg-white/70 p-8 text-center">
                ...
                <div className="mt-3 text-sm font-semibold text-black">Events coming soon</div>
            ...
            {gallery.length > 0 ? (
              <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                {gallery.map((g, i) => (
                  <ScrollReveal key={g.id} delay={i * 60}>
                    <div className="group relative aspect-square overflow-hidden rounded-3xl border border-black/10 bg-white/70">
                      <Image
                        src={g.image_url}
                        alt={g.caption}
                        fill
              ...
              <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="flex aspect-square items-center justify-center rounded-3xl border border-black/10 bg-white/70 p-4 text-center">
                    <div>
                      <div className="text-3xl">📷</div>
                      <div className="mt-2 text-xs font-semibold text-black/60">Gallery coming soon</div>
                    </div>
                  </div>
                ))}
              </div>
