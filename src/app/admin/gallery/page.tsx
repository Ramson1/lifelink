    ...
    <div className="space-y-6">
      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Gallery</h1>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Manage media gallery images displayed on the website.</p>
        </div>
        {canCrud && (
      ...
      {showForm && canCrud && (
        <form onSubmit={handleSubmit(onSubmit)} className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-700 dark:text-slate-300">Caption</label>
              <input {...register("caption")} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white" placeholder="e.g. Annual conference 2024" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-700 dark:text-slate-300">Category</label>
              <select {...register("category")} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white">
                {CATEGORIES.map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
              </select>
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-700 dark:text-slate-300">Sort Order</label>
              <input type="number" {...register("sort_order", { valueAsNumber: true })} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
            </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
        <div className="flex items-center gap-2 border-b border-slate-200 px-6 py-4 dark:border-slate-700">
          ...
          <h2 className="text-sm font-semibold text-slate-900 dark:text-white">All Images ({items.length})</h2>
        </div>
        {loading ? (
          <div className="flex items-center justify-center py-16"><Loader2 className="h-5 w-5 animate-spin text-slate-500" /></div>
        ) : items.length === 0 ? (
          <div className="px-6 py-16 text-center text-sm text-slate-500 dark:text-slate-400">No gallery images yet. Add your first image above.</div>
        ) : (
          <div className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((g) => (
              <div key={g.id} className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900">
                ...
                <div className="p-3">
                  <div className="flex items-center gap-2">
                    <p className="text-xs font-semibold text-slate-900 truncate flex-1 dark:text-white">{g.caption || "No caption"}</p>
                    {!g.is_active && <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-500 dark:bg-slate-700 dark:text-slate-400">Inactive</span>}
                  </div>
                  <div className="mt-1 flex items-center gap-2 text-[10px] text-slate-400">
                    <span className="rounded-full border border-slate-200 px-1.5 py-0.5 font-semibold dark:border-slate-600 dark:text-slate-300">{categoryLabel(g.category)}</span>
                  </div>
                  <div className="mt-2 flex items-center gap-1">
                    {canCrud && (
                      <>
                        <button onClick={() => toggleActive(g)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-700 dark:hover:text-slate-300" title={g.is_active ? "Deactivate" : "Activate"}>
                          {g.is_active ? <Eye className="h-3.5 w-3.5" /> : <EyeOff className="h-3.5 w-3.5" />}
                        </button>
                        <button onClick={() => openEdit(g)} className="rounded-lg p-1 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30" title="Edit"><Pencil className="h-3.5 w-3.5" /></button>
                        <button onClick={() => onDelete(g.id)} disabled={deletingId === g.id} className="rounded-lg p-1 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 disabled:opacity-30" title="Delete">
