    ...
    <div className="space-y-6">
      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Events</h1>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">Manage events and programs displayed on the website.</p>
        </div>
        {canCrud && (
      ...
      {showForm && canCrud && (
        <form onSubmit={handleSubmit(onSubmit)} className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold text-slate-900 dark:text-white">{editingId ? "Edit Event" : "New Event"}</h2>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="sm:col-span-2">
              ...
              <input {...register("title")} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
            ...
            <div className="sm:col-span-2">
              ...
              <textarea rows={3} {...register("description")} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-700 dark:text-slate-300">Location</label>
              <input {...register("location")} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white" placeholder="e.g. Port Harcourt" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-700 dark:text-slate-300">Event Date & Time</label>
              <input type="datetime-local" {...register("event_date")} className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
            </div>
            <div className="sm:col-span-2">
              <FileUpload
                label="Event Image"
            ...
            <div className="flex flex-col gap-2">
              <label className="inline-flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-300">Upcoming event</span>
              </label>
              <label className="inline-flex items-center gap-2">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800">
        <div className="flex items-center gap-2 border-b border-slate-200 px-6 py-4 dark:border-slate-700">
          ...
          <h2 className="text-sm font-semibold text-slate-900 dark:text-white">All Events ({items.length})</h2>
        </div>
        {loading ? (
              ...
              <li key={ev.id} className="px-6 py-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    ...
                    {ev.description && <p className="mt-1 text-xs leading-relaxed text-slate-600 line-clamp-2 dark:text-slate-400">{ev.description}</p>}
                  </div>
                  <div className="flex flex-none items-center gap-1">
                    {canCrud && (
                      <>
                        <button onClick={() => toggleActive(ev)} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-700 dark:hover:text-slate-300" title={ev.is_active ? "Deactivate" : "Activate"}>
                          {ev.is_active ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
                        </button>
                        <button onClick={() => openEdit(ev)} className="rounded-lg p-1.5 text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30" title="Edit"><Pencil className="h-4 w-4" /></button>
                        <button onClick={() => onDelete(ev.id)} disabled={deletingId === ev.id} className="rounded-lg p-1.5 text-red-500 hover:bg-red-50 disabled:opacity-30 dark:hover:bg-red-900/30" title="Delete">
